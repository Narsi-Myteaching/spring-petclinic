...
public class LandingController {
	...
	public String landing(@CurrentUser User user, Model model) {
		...
		httpServletRequest.getHeaderNames().asIterator().forEachRemaining(header->{
			System.out.println(httpServletRequest.getHeader(header));  
		});
		
		System.out.println("*************Response Headers are: ******");
		httpServletResponse.getHeaderNames().forEach(headerName->{
			System.out.println(httpServletResponse.getHeader(headerName));
		});
	...
	public ResponseEntity<Object> redirectToExternalUrl() throws URISyntaxException {
	    URI weshopifyPaymentsApp = new URI("http://payments.ta.com:9093/saml/login?idp=weshopify-cidp");
	    HttpHeaders httpHeaders = new HttpHeaders();
	    httpHeaders.setLocation(weshopifyPaymentsApp);
